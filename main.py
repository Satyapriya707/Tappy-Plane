...
class Game:
    ...
    def load_data(self):
        ...
        self.spritesheet = Spritesheet(path.join(img_dir, SPRITESHEET[0]))
        # background image
        self.bg_img = pg.image.load(path.join(img_dir, 'background.png')).convert()
        self.bg_img = pg.transform.scale(self.bg_img, (800,600))
        # intro image
        self.intro_img = pg.image.load(path.join(img_dir, 'textGetReady.png')).convert()
        ...
        # game over image 
        self.go_img = pg.image.load(path.join(img_dir, 'textGameOver.png')).convert()
...
##                    self.player.jump_cut()
    ...
    def show_start_screen(self):
        ...
        self.button = pg.image.load(path.join(img_dir, 'buttonLarge.png')).convert()
        ...
        #self.plane = self.spritesheet.get_image(216, 1878, 88, 73)
        self.plane = pg.image.load(path.join(img_dir, 'planeRed1.png')).convert()
        self.plane.set_colorkey(BLACK)
        self.screen.blit(self.plane, (WIDTH * .40,HEIGHT * .5))

        #self.tap = self.spritesheet.get_image(418, 1350, 59, 59)
        self.tap = pg.image.load(path.join(img_dir, 'tapTick.png')).convert()
    ...
    def show_go_screen(self):
        ...
        self.gold = pg.image.load(path.join(img_dir, 'medalGold.png')).convert()
        self.gold.set_colorkey(BLACK)
        self.silver = pg.image.load(path.join(img_dir, 'medalSilver.png')).convert()
        self.silver.set_colorkey(BLACK)
        self.bronze = pg.image.load(path.join(img_dir, 'medalBronze.png')).convert()
